...
const BOT_NAME = configEnv.discord.botName;
const BOT_AVATAR = configEnv.discord.botAvatar;

export class DiscordLog {
  static async #sendRaw(payload) {
        ...
        try {
          ...
          if (remaining && parseInt(remaining) === 0) {
            const resetAfter = parseFloat(
              response.headers["x-ratelimit-reset-after"] || 1
            );
          ...
          if (error.response?.status === 429) {
            retryAfter =
              (error.response.headers["retry-after"] || retryAfter / 1000) *
                1000 +
              200;
            console.warn(
              `[Discord Webhook] 429 - Chờ ${
                retryAfter / 1000
              }s rồi thử lại... (${i + 1}/5)`
            );
          ...
          if (i === 4) {
            console.error(
              "[Discord Webhook] Gửi thất bại sau 5 lần retry:",
              error.message
            );
  ...
  static async sendLogError(message, info = {}) {
    const errorStack = info.stack || info.error?.stack || "No stack trace";
    const errorMsg =
      info.message || info.error?.message || "No additional message";

    const errorCode = info.code || info.error?.code || "N/A";
  ...
  static async sendLogWarning(message, info = {}) {
    await this.#send({
      title: "⚠️ Cảnh Báo",
      description: "```fix\n" + message + "\n```",
      color: 0xf39c12, // Cam
  ...
  static async sendLogInfo(message, info = {}) {
    await this.#send({
      title: "ℹ️ Thông Báo",
      description: "```yaml\n" + message + "\n```",
  ...
  static async sendLogSuccess(message, info = {}) {
    await this.#send({
      title: "✅ Thành Công",
      description: "```diff\n+ " + message.replace(/\n/g, "\n+ ") + "\n```",
